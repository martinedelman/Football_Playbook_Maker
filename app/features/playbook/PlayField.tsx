...
export type PlayFieldProps = {
  ...
  onPlayerPositionChange?: (playerId: string, position: Coordinate) => void;
  onPlayerPointerDown?: (playerId: string) => void;
};
...
export function PlayField({
  ...
  const translatePointerToField = (event: React.PointerEvent<SVGSVGElement>) => {
  ...
  const handlePointerMove = (event: React.PointerEvent<SVGSVGElement>) => {
    if (!draggingPlayerId || !onPlayerPositionChange) return;
    const position = translatePointerToField(event);
    event.preventDefault();
    onPlayerPositionChange(draggingPlayerId, position);
  };
        ...
        return (
          <g key={player.id}>
            ...
            <circle
              ...
              strokeWidth={3}
              onPointerDown={(event) => {
                if (!onPlayerPositionChange) return;
                event.stopPropagation();
                event.preventDefault();
                setDraggingPlayerId(player.id);
                onPlayerPointerDown?.(player.id);
              }}
              style={onPlayerPositionChange ? { cursor: "grab" } : undefined}
            />
            <text
              x={player.position.x}
