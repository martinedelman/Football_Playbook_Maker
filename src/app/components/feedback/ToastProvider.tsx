...
function Toast({ toast, onClose }: { toast: ToastItem; onClose: (id: number) => void }) {
  ...
  useEffect(() => {
    if (toast.status === FeedbackStatus.ERROR) return;
    const timeout = window.setTimeout(() => onClose(toast.id), 5000);
    return () => window.clearTimeout(timeout);
  }, [onClose, toast.id, toast.status]);
  ...
  return (
    <div
      ...
      style={{ transform: `translate(${offset.x}px, ${offset.y}px)`, opacity: Math.max(0.35, 1 - Math.abs(offset.x) / 240) }}
